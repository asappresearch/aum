...
class Runner(object):
    ...
    def generate_aum_details(self, load=None):
        ...
        result["AUM_LIB"] = torch.tensor(aum_list)
        ...
        # Get a mask for epochs that we recorded data for
        # This is for if we stopped training early
        mask = train_data["correct_confs"].float().sum(-1).gt(0)
        mask[:start_epoch] = False

        # Get the losses
        losses = train_data["correct_confs"][mask].float().clamp_min(1e-10).log().mul(-1)

        # Add Loss
        result["Loss"] = losses[-1]

        # Add confidence
        result["Correct Logit"] = train_data["correct_logits"][mask][-1]
        result["Incorrect Logit"] = train_data["incorrect_logits"][mask][-1]
        result["Margin"] = (result["Correct Logit"].float() -
                            result["Incorrect Logit"].float()).half()
        result["AUM"] = torch.sub(train_data["correct_logits"][mask].float(),
                                  train_data["incorrect_logits"][mask].float()).mean(0)
    ...
    def train(self,
        ...
        aum_calculator = AUMCalculator(save_dir=self.savedir)
        ...
        ## OLD WAY OF DOING THINGS ############
        #######################################
        train_data["preds"] = torch.zeros(num_epochs, len(self.train_set), dtype=torch.long)
        train_data["correct_confs"] = torch.zeros(num_epochs, len(self.train_set), dtype=torch.half)
        train_data["correct_logits"] = torch.zeros(num_epochs,
                                                   len(self.train_set),
                                                   dtype=torch.half)
        train_data["incorrect_confs"] = torch.zeros(num_epochs,
                                                    len(self.train_set),
                                                    dtype=torch.half)
        train_data["incorrect_logits"] = torch.zeros(num_epochs,
                                                     len(self.train_set),
                                                     dtype=torch.half)
        train_data["top_incorrect"] = torch.zeros(num_epochs, len(self.train_set), dtype=torch.long)
        ...
        for epoch in range(num_epochs):
            train_results = self.train_epoch(
                ...
                correct_confs_storage=train_data["correct_confs"],
                correct_logits_storage=train_data["correct_logits"],
                incorrect_confs_storage=train_data["incorrect_confs"],
                incorrect_logits_storage=train_data["incorrect_logits"],
                preds_storage=train_data["preds"],
                top_incorrect_storage=train_data["top_incorrect"],
                aum_wtr=aum_wtr,
    ...
    def train_epoch(self,
                    ...
                    num_workers=0,
                    aum_calculator=None,
                    correct_confs_storage=None,
                    correct_logits_storage=None,
                    incorrect_confs_storage=None,
                    incorrect_logits_storage=None,
                    top_incorrect_storage=None,
                    preds_storage=None,
                    aum_wtr=False,
                    rand_weight=False):
        stats = ["error", "loss"]
        ...
        for inputs, targets, indices in loader:
            ...
            outputs = model(inputs)
            probs = torch.softmax(outputs, dim=-1)  # OLD WAY OF DOING THINGS
            losses = self.loss_func(outputs, targets, reduction="none")
            ...
            # Get the correct logit/conf
            index = torch.arange(targets.size(0))
            correct_logits = outputs[index, targets]
            correct_confs = probs[index, targets]

            # Get the incorrect logit/conf
            one_hot = torch.nn.functional.one_hot(targets,
                                                  num_classes=self.num_classes).type_as(probs)
            top_incorrect = (probs - one_hot).argmax(dim=-1)
            incorrect_logits = outputs[index, top_incorrect]
            incorrect_confs = probs[index, top_incorrect]
            ...
            # Record losses
            # TODO: Rip this out and replace with AUM
            if correct_confs_storage is not None:
                correct_confs_storage[epoch, indices] = correct_confs.detach().cpu().half()
            if correct_logits_storage is not None:
                correct_logits_storage[epoch, indices] = correct_logits.detach().cpu().half()
            if incorrect_confs_storage is not None:
                incorrect_confs_storage[epoch, indices] = incorrect_confs.detach().cpu().half()
            if incorrect_logits_storage is not None:
                incorrect_logits_storage[epoch, indices] = incorrect_logits.detach().cpu().half()
            if top_incorrect_storage is not None:
                top_incorrect_storage[epoch, indices] = top_incorrect.detach().cpu()
            if preds_storage is not None:
                preds_storage[epoch, indices] = preds.detach().cpu()
